...
Karafka Web UI injects its group, which the mapper also altered. To mitigate it, set the consumer group explicitly for `processing` and `commanding`:
...
Karafka::Web.setup do |config|
  ...
  # Configure commanding with the desired group - you can use same as for processing
  # in most of the cases
  config.commanding.consumer_group = 'my-app_karafka_web'
end
...
Until Karafka `2.3` if you wanted to stop the Pro Iterator, you could just exit it's main messages loop like so:
