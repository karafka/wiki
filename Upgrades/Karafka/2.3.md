...
As always, make sure you have upgraded to the most recent version of `2.2` before upgrading to `2.3`.

Also, remember to read and apply our standard [upgrade procedures](Upgrades-Upgrading).

## Web UI Upgrade

Karafka `2.3` works only with Web UI `>= 0.8.0`. Follow the [Web UI](Upgrades-Web-UI-0.8) upgrade procedure.
...
While this change has already been introduced in `2.3.0` as a fix, we wanted to make you aware that all the CLI inclusion and exclusion options, such as `--include-consumer-groups` or `--exclude-consumer-groups` expect a list of arguments separated by a **comma** and **NOT** by space:
...
Karafka no longer relies on `concurrent-ruby`. If you have indirectly relied on it being included in your applications, you must add it as a direct dependency yourself.
