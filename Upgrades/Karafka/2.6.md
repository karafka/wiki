...
### Reading Partition Offsets From a Consumer's Own Connection

`Karafka::Connection::Client#read_partition_offsets` (and `Connection::Proxy#read_partition_offsets`) is a separate, distinct feature from the Admin API method above. It runs the same batched `ListOffsets` query but on a running consumer's own connection instead of opening a dedicated Admin client, and forwards that connection's configured `isolation.level`. As with the Admin variant, `:latest` resolves to the high watermark regardless of isolation level, so it can include uncommitted messages on topics with in-flight transactions.

### Running Admin Operations on an External Client

`Karafka::Admin.new` (and `Admin::ConsumerGroups`, `Admin::Topics`) now accepts an `external_client:` keyword argument, letting Admin operations run on an already-open client instead of bootstrapping a new one per call. The external client's lifecycle stays fully with its caller - it is never configured, started, or closed by the Admin instance:
...
## Liveness: Detecting a Frozen Join State (`stability_ttl`)

`Kubernetes::LivenessListener` and `Pro::Swarm::LivenessListener` gained a `stability_ttl` setting that detects a consumer group frozen in a single non-`"steady"` librdkafka `cgrp.join_state` (for example, stuck in `CompletingRebalance`). The timer resets on every join state transition, so only a group that is continuously frozen is flagged - a healthy sequence of transitions never trips it.

`stability_ttl` requires `statistics.interval.ms` to be enabled; without it, this check has no effect. It defaults to twice the maximum `max.poll.interval.ms` across active subscription groups. On breach, the listener reports `stability_ttl_exceeded: true` over HTTP (status `500`) and as swarm status code `4`, clearing once the group returns to `"steady"`.
...
No action is required to upgrade - `stability_ttl` has a computed default and does not need to be set explicitly.
