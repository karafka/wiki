...
Before reading this article, ensure you have read the [Karafka framework 2.0 announcement](https://mensfeld.pl/2022/08/karafka-framework-2-0-announcement/) blog post so you are familiar with the overall scope of changes.

There are many aspects of Karafka upgrade that are specific to your application. While we may not have covered all of the cases here, **do not** hesitate to ask questions either via [Slack](https://slack.karafka.io/) or by creating a [Github issue](https://github.com/karafka/karafka/issues/new/).
...
Karafka does **not** provide warranties, that the same consumer will always run in the same Ruby thread, hence code like this:
...
Karafka 2.0 is powered by librdkafka. Because of that, we have decided to split the settings into two sections:
...
Read the [Deployment](Infrastructure-Deployment) documentation to see appropriate configuration for given environment.
...
Karafka 2.0 introduces Ruby on Rails integration via `Rails::Railte` without needing extra configuration.
...
You can still use instrumentation hooks if you need to perform any consumer, not related actions, but be aware that this code will not run in the same thread as the consumption.
...
Topic mapping is no longer supported. Prefix all of your topic names with the env `KAFKA_PREFIX`.

You can read more about integrating Karafka 2.0 with Heroku [here](Infrastructure-Deployment#heroku).

## Pidfile and daemonization support has been removed

Quote from [Mike](https://github.com/mperham/sidekiq/issues/4045):

- Do not daemonize, start Karafka with a process supervisor like systemd, upstart, foreman, etc.
...
Use [Waterdrop](https://github.com/karafka/waterdrop) to produce messages via `Karafka.producer`:
...
[Here](Basics-Consuming-Messages#consuming-messages-one-at-a-time) you can find more details about this.

## Dependency changes

Karafka `2.0` no longer uses any of the `dry-rb` ecosystem libraries. If you have relied on them indirectly via Karafka, you will have to define them in your `Gemfile`.
...
The recommendation here is to revisit your current integration and align it with the events published by Karafka and to follow the `2.0` [instrumentation guidelines document](Infrastructure-Monitoring-and-Logging).
