...
Karafka 2.5 introduces several breaking changes, most of which align naming conventions with other Kafka ecosystem tools, such as Kafka Streams and Apache Flink. Most of these changes are low-risk, configuration-alignment renames, but the Pro DLQ routing change alters DLQ message partitioning behavior and can affect processing flows - see [Impact Assessment](#impact-assessment) below.
...
As always, make sure you have upgraded to the most recent version of `2.4` before upgrading to `2.5`.
...
If you have customized admin retry behavior, update your configuration to use the new time-based approach. For example, if you previously set `max_attempts = 30` with the default 1-second wait time (30 seconds total), you would now set `max_retries_duration = 30_000` to maintain equivalent behavior.
...
- **Low Risk**: The header prefix rename, Web UI/recurring tasks topic configuration structure changes, Kubernetes health check response format, and admin configuration option renames are purely naming or format related and do not affect consumer group names, processing logic, or system stability. Simple search and replace operations should be enough for most codebases.
- **Mid-to-Critical Risk**: The Pro DLQ message key and routing change alters DLQ message partitioning behavior - see [Critical Partitioning Change](#impact-on-your-code) above for what this can affect and how to preserve the previous behavior if needed.
