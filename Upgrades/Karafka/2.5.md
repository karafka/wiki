...
The Pro DLQ now preserves the original message key instead of overwriting it with the source partition ID. This makes sure that the original routing logic is maintained when messages are dispatched to the DLQ topic, preserving the source partition ordering as well.
...
The Pro DLQ now uses `partition_key` for consistent partition targeting while preserving the original message key. This makes sure that:
...
- **Migration**: Simple search and replace operations should be enough for most codebases.
