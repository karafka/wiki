...
!!! tip "Version Compatibility Requirement"
    ...
    Karafka Web UI `1.0` requires Karafka `2.6` - these components must be upgraded together. Attempting to run Web UI `1.0` with older versions of Karafka will result in compatibility errors. Please make sure you upgrade both components as part of the same deployment process. Karafka `2.6` Upgrade Guide can be found [here](Upgrades-Karafka-2.6).
