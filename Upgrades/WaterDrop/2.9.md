...
!!! note "Experiencing Issues?"
    ...
    If you experience any issues with the `:fd` polling mode, [open an issue](https://github.com/karafka/waterdrop/issues/new) on the WaterDrop GitHub repository.

The `:thread` polling mode remains fully supported as of the current WaterDrop release, with no deprecation warning when it is used.
