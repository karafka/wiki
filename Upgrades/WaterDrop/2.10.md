...
Each tombstone message requires a `topic`, a `key` (non-empty String), and a `partition` (non-negative Integer). The `payload` is always set to `nil`. See the [Usage](WaterDrop-Usage#tombstones) documentation for details and examples.
