...
- `#commit_offsets`: This method is asynchronous. It sends a request to the Kafka brokers to commit the offsets and does not wait for the brokers to acknowledge it. Instead, it returns immediately, which allows your consumer to continue processing other messages without delay. It returns a Boolean right away, but because the call is async, that Boolean only reflects whether the local commit request was issued without hitting a known failure (such as having already lost the partition) - it does not confirm the brokers actually completed the commit, and it can still return `true` despite an involuntary partition revocation happening around the same time. If you need to know whether you still own the partition, check the `#revoked?` method afterward.
