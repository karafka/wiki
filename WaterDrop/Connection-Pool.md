...
| `connection_pool.shutdown` | Emitted exactly once per global-pool shutdown, from within the instance-level `#shutdown` while still active; `ConnectionPool.active?` returns `true` if called from this handler.                 |
