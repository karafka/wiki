...
- A raised number of `error_occurred` notifications with `type: app.stopping.error` (see [Monitoring Forceful Shutdowns](#monitoring-forceful-shutdowns) below).
...
Applying the mitigations above (a higher `shutdown_timeout`, static membership, slower deploys, or KIP-848) should drive this count down substantially. When a forceful shutdown does occur, Karafka `2.5.7+` logs the active listeners, alive workers, and in-processing jobs that were still blocking, which helps you distinguish a slow job from a stuck rebalance - see [Forceful Shutdown](Infrastructure-Signals-and-States#forceful-shutdown).
