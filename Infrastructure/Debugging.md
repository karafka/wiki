...
The limit applies to the whole batch polled by a subscription group collectively, not to any single topic. When a subscription group subscribes to many topics, no single topic needs to be slow for the combined batch to exceed `max.poll.interval.ms`, which is why the error names only a consumer group and not a topic. For a detailed explanation and how to resolve it, see [Why do I get a `max.poll.interval.ms` exceeded error even though no single topic or message is slow and my metrics look fine?](Basics-FAQ-Consuming#why-do-i-get-a-maxpollintervalms-exceeded-error-even-though-no-single-topic-or-message-is-slow-and-my-metrics-look-fine).
...
- Consider using the [Long-Running Jobs](Pro-Consumer-Groups-Long-Running-Jobs) feature.
...
- [Consumer Lag Spikes and Offset Resets](Consumer-Groups-Offset-Rewind) - Diagnosing and resolving the pattern where consumer lag jumps to the retention ceiling after a broker failure or rebalance
