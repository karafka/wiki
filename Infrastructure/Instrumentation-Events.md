...
Below is a list of the instrumentation events available in Karafka. Depending on which Karafka extensions and additional engines you use, this might not be the complete list of all the events: some events are registered dynamically at runtime (for example `recurring_tasks.task.executed`, registered by Karafka Pro's Recurring Tasks). Use `#available_events` on a fully loaded Karafka system to determine all the events you can use.
