...
## 0.29.1 (Unreleased)
- [Enhancement] Add `Admin#delete_records`, wrapping librdkafka's `DeleteRecords` admin API. Deletes all messages in a partition up to (but not including) a given offset - the programmatic equivalent of `kafka-delete-records.sh` - useful for GDPR/right-to-erasure compliance, clearing out poison messages, or retention cleanup outside of the configured time/size-based policy. Accepts `:end` as the offset to delete all data currently in a partition.
