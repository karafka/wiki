...
- [Fix] Emit `connection_pool.shutdown` only once when shutting down the global connection pool. `WaterDrop::ConnectionPool.shutdown`/`.close` delegates to the instance-level `#shutdown` (which already instruments the event with `pool: self`) and then instrumented `connection_pool.shutdown` a second time itself, so subscribers saw the event twice per global shutdown. The class-level method no longer re-emits; the instance `#shutdown` is the single source of truth (a direct instance `#shutdown` was, and still is, emitted exactly once). Note: the event now fires while `default_pool` is still set (from within the delegated instance shutdown) rather than after it is cleared, so a subscriber inspecting `ConnectionPool.active?` inside the handler will see `true`.
