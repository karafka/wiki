...
## Unreleased
- **[Feature]** Aggregate the Health views per topic instead of per partition so they stay usable with many topics/partitions. The top level shows one summary row per topic (lag, max/avg lag, a "skewed" flag, LSO risk, paused partitions) with drill-down to the per-partition Overview/Lags/Offsets/Changes/Cluster Lags of a topic. Rows are highlighted with a colored left border when a partition's lag (or a topic's average partition lag) crosses `config.ui.health.lags.high_threshold` (red at the threshold, yellow at `config.ui.health.lags.warning_ratio` of it); skewed topics, topics with paused partitions and individual paused partitions are also flagged yellow, so a runaway lag or a paused partition is visible at a glance. Skew detection is configurable via `config.ui.health.lags.skew_threshold` and `config.ui.health.lags.skew_minimum` (#112).
