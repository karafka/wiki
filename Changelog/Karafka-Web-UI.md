...
- [Fix] Validate server-side that a new partition count is actually greater than the topic's current one (Pro). `create_partitions` sets the total, so a value at or below the current count was accepted by the form and only rejected by the broker as a raw error; the form now re-renders with a field error, matching the `min` and helper text it already showed. The form also no longer pre-fills the field with an invalid default, rendering empty with its placeholder instead.
