...
## Unreleased
- [Maintenance] Stop the recurring `/topics` link-validator flake: the cluster distribution/replication specs stub `ClusterInfo.fetch` with fabricated multi-broker metadata, which makes the incidentally-crawled `/topics` page intermittently 500. Skip `/topics` when crawling from the cluster views (it is covered by its own specs), matching the existing `/explorer` exclusion.
- [Fix] Do not 500 the Errors views when the errors topic contains a foreign or malformed message (for example one a user published to it by mistake). Such an entry now renders a "not a valid Karafka error report" placeholder (distinct from the compacted/removed-offset one) that keeps a Details link opening the raw message in the Explorer (Pro), instead of raising (#1243).
- [Maintenance] Reorganize the Pro UI `Lib` feature pipelines into domain namespaces that mirror the controllers/routes: `Lib::Consumers::Commands`, `Lib::Explorer::{Publishing,Republishing,Search}`, and `Lib::Topics::{Configuring,Creation,Repartitioning}` (was a flat `Lib::Commands`, `Lib::Publishing`, `Lib::Search`, etc.). Internal only; no behavior change.
...
- [Enhancement] Dispatch consumer commanding requests (pause, resume, seek, quiet, stop, trace) through the acked (`acks: 1`) producer so the broker confirms receipt, instead of the fire-and-forget (`acks: 0`) reporting producer that can silently drop a command. Adds a `Karafka::Web.producers` facade (`producers.default`, an alias of `Karafka::Web.producer`, and `producers.acked`) as the single source of truth for user-initiated produces (also used by the Explorer publish/republish flows) (#1241).
...
- [Maintenance] Extract the Explorer republish flow into `Lib::Republishing` (Normalizer/Contract/Transform, reusing `Publishing::Dispatcher`), mirroring the publish extraction. The republish form now validates that the target topic exists, an explicit target partition is in range, and the payload is consumable by the target topic's deserializer (skippable), re-rendering with errors instead of failing on produce (#1244).
