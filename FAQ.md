...
!!! tip "Consider KIP-848 for Better Rebalancing"

    If you're running Kafka 4.0+, consider migrating to the new KIP-848 consumer protocol (`group.protocol: 'consumer'`) which handles rebalancing more efficiently and avoids many of these issues.
...
It typically means that your `group.instance.id` is not unique within your consumer group. You must always ensure that the value you assign to `group.instance.id` is unique within the whole consumer group, not unique per process or machine.
...
!!! warning "Complete Shutdown Required for Protocol Migration"

    When migrating to KIP-848 consumer protocol (`group.protocol: 'consumer'`), you'll also need to perform a complete shutdown of all consumers before switching, as the new protocol is incompatible with the older assignment strategies.
...
This issue is typically caused by a gem conflict related to the Thor gem version. Current versions of Karafka no longer depend on Thor, so upgrading to the latest stable version should resolve this issue.
